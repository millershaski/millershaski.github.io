...
window.addEventListener('DOMContentLoaded', event => {
    ...
    RandomizeSuccessEntry(); // randomize when the page loads too
});
...
document.getElementById("ageRangeForm").addEventListener("submit", function(event){   
    ...
    document.getElementById("averageSalary").textContent = "Number of entries: " + entryCount + ", Average Salary: $" + averageSalary;
});



function RandomizeSuccessEntry()
{
    let randomElement = tableData[Math.floor(Math.random() * tableData.length)];
    
    let featuredEntry = document.getElementById("featuredEntry");
    if(featuredEntry == null)
    {
        console.log("Unable to find element with ID: featuredEntry");
        return;
    }

    // name: "Serge Baldwin", position: "Data Coordinator", office: "Singapore", age: 64, startDate: "2012/04/09", salary: "$138,575" },

    featuredEntry.innerHTML = randomElement.name + " : " + randomElement.age + 
                                "<br>" + randomElement.office + " (" + randomElement.position +")" +
                                "<br>" + randomElement.salary;
}